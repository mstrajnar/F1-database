...
F1_FACTI = [
    "Monaco GP je najpočasnejša dirka v koledarju F1.",
    "Pit stop ekipe Red Bull traja manj kot 2 sekundi.",
    "Formula 1 bolid ustvari več G-sile kot fighter jet pri zavijanju.",
    "Lewis Hamilton ima največ zmag v zgodovini F1.",
    "Max Verstappen je postal najmlajši zmagovalec dirke F1.",
    "F1 bolid lahko vozi po stropu pri dovolj visoki hitrosti.",
    "Pnevmatike izgubijo ogromno oprijema že po nekaj krogih.",
    "DRS pomeni Drag Reduction System.",
    "Monza velja za tempelj hitrosti.",
    "Motorji F1 dosežejo preko 15.000 RPM.",
    "Charles Leclerc je bil član Ferrari Driver Academy.",
    "Safety Car lahko popolnoma spremeni strategijo dirke.",
    "Fernando Alonso je debitiral v F1 leta 2001.",
    "Lando Norris je eden najbolj priljubljenih voznikov nove generacije.",
    "Formula 1 uporablja telemetrijo v realnem času.",
]
...
app.layout = html.Div(style=SLOG_OZADJE, children=[
    ...
    html.Div(
        ...
        children=[
            ...
            html.Div(className="f1-loader-track"),

            html.Div(className="f1-loader-car", children="🏎️"),

            html.Div(
                id="loading-fact",
                className="f1-loading-fact",
                children="Pripravljam F1 podatke ..."
            ),

            html.Div(
                "Nalagam podatke iz OpenF1 ...",
                className="f1-loading-subtitle"
            ),
        ],
    ),

    html.Div(id="rezultati-vsebina"),
...
@app.callback(
    Output("loading-fact", "children"),
    Input("loading-fact-interval", "n_intervals"),
)
def posodobi_loading_fact(n):
    return random.choice(F1_FACTI)

@app.callback(
    Output("loading-overlay", "style"),
    Output("loading-fact-interval", "disabled"),
    Input("btn-poisci", "n_clicks"),
    Input("rezultati-vsebina", "children"),
    prevent_initial_call=True,
)
def loading_overlay(n_clicks, rezultati):

    trigger = ctx.triggered_id

    if trigger == "btn-poisci":
        return (
            {
                "display": "flex"
            },
